...
The CLI tool itself can be updated by running the installer again:
...
The installer will update the CLI to the latest version if one is available.

You can also check for CLI updates without installing:

```bash
desterlib update-check
```

This will show you if a newer version is available and provide the update command.

## Updating the Server

### CLI Installation (Recommended Method)

If you installed via the CLI installer:
...
- [Aggregated Changelog](https://docs.dester.in/changelog) on docs site
