...
function MediaDetails() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.media.getById(mediaId);
  ...
  const playMovie = () => {
    ...
    const streamUrl = `${baseUrl}/api/media/stream/movie/${mediaId}`;
    playVideo(streamUrl);
  };
  ...
  const playEpisode = (
    ...
    const baseUrl = import.meta.env.VITE_API_URL || "http://localhost:3000";
    const streamUrl = `${baseUrl}/api/media/stream/episode/${mediaId}/${seasonNumber}/${episodeNumber}`;
  ...
  // Get cast members
  const cast = data.people?.filter((p) => p.role === "ACTOR") || [];
  const directors = data.people?.filter((p) => p.role === "DIRECTOR") || [];
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col lg:flex-row gap-8 items-end">
              ...
              <div className="flex-1 space-y-4 pb-2">
                ...
                {/* Action Buttons */}
                {data.type === "MOVIE" && data.movie?.filePath && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
        ...
        {data.type === "TV_SHOW" && data.tvShow?.seasons && (
          ...
          >
            ...
            <div className="space-y-3">
              {data.tvShow.seasons.map((season) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        setExpandedSeasonId(isExpanded ? null : season.id)
                      }
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              {directors.map((director) => (
                ...
                >
                  <span className="text-white/80">{director.person?.name}</span>
                </div>
              ))}
            </div>
          </motion.section>
