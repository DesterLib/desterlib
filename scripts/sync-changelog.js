  ...
  if (fs.existsSync(pkg.changelogPath)) {
    try {
      ...
      if (!changelogContent || !changelogContent.trim()) {
        log(`⚠️  ${pkg.name} changelog is empty`, "yellow");
      } else {
        log(`✓  Found ${pkg.name} changelog`, "green");
      }
    } catch (error) {
      log(`❌ Error reading ${pkg.name} changelog: ${error.message}`, "red");
    }
  } else {
    log(
      `⚠️  Warning: ${pkg.name} changelog not found at ${pkg.changelogPath}`,
      "yellow"
    );
  }
...
try {
  log("\n📝 Syncing package changelogs to docs", "blue");
  log("─".repeat(50), "blue");
  ...
  log("");
  log(`✅ Synced ${syncedCount} changelog(s) to docs`, "green");
  log(`✓  ${contentCount} changelog(s) have content`, "yellow");
  log("");
} catch (error) {
  log(`❌ Error syncing changelogs: ${error.message}`, "red");
  log(error.stack, "red");
  process.exit(1);
}
