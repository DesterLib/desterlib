...
Write-Output ""
Write-ColorOutput Cyan "Downloading DesterLib configuration files..."
...
try {
    ...
    $composeContent = $composeContent -replace "`r`n", "`n" -replace "`r", "`n"
    [System.IO.File]::WriteAllText($composePath, $composeContent, [System.Text.UTF8Encoding]::new($false))
    
    Write-ColorOutput Green "[OK] docker-compose.yml downloaded"
} catch {
    Write-ColorOutput Red "[X] Failed to download docker-compose.yml: $_"
