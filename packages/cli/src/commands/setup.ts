...
import { getInstallationDir, isInstalled, validatePath, removeDirectory, ensureDirectory } from '../utils/paths.js';
...
export async function setupWizard(): Promise<void> {
  try {
      ...
      const { action } = await inquirer.prompt([
        {
          type: 'list',
          name: 'action',
          message: chalk.yellow('⚠️  DesterLib configuration already exists. What would you like to do?'),
          choices: [
    ...
    console.log(chalk.white('  Media Path:       ') + chalk.cyan(config.mediaPath));
    console.log(chalk.white('  API Port:         ') + chalk.cyan(config.port.toString()));
    console.log(chalk.white('  Database User:    ') + chalk.cyan(config.postgresUser));
    console.log(chalk.white('  Database Name:    ') + chalk.cyan(config.postgresDb));
    console.log(chalk.white('  Database Password:') + chalk.cyan('***hidden***'));
    console.log('');
    console.log(chalk.gray('Note: TMDB API key and JWT secret are configured in-app, not here.'));
    console.log('');
...
function displaySuccessMessage(port: number, installDir: string): void {
  ...
  console.log(chalk.gray('  ├─') + chalk.cyan(` API Server:      http://localhost:${port}`));
  console.log(chalk.gray('  ├─') + chalk.cyan(` API Docs:        http://localhost:${port}/api/docs`));
  console.log(chalk.gray('  ├─') + chalk.cyan(` Health Check:    http://localhost:${port}/health`));
  console.log(chalk.gray('  └─') + chalk.cyan(` WebSocket:       ws://localhost:${port}/ws`));
  
  console.log(chalk.bold('\n📱 Next Steps:'));
  console.log(chalk.gray('  1.') + chalk.white(' Open the API docs and configure your media library'));
  console.log(chalk.gray('  2.') + chalk.white(' Download the DesterLib mobile/desktop app'));
  console.log(chalk.gray('  3.') + chalk.white(` Connect to: http://localhost:${port}`));
  
  console.log(chalk.bold('\n📂 Installation Location:'));
  console.log(chalk.gray('  ') + chalk.white(installDir));
  
  console.log(chalk.bold('\n🛠️  Useful Commands:'));
  console.log(chalk.gray('  ├─') + chalk.white(' Stop:    ') + chalk.cyan(`cd ${installDir} && docker-compose down`));
  console.log(chalk.gray('  ├─') + chalk.white(' Restart: ') + chalk.cyan(`cd ${installDir} && docker-compose restart`));
  console.log(chalk.gray('  ├─') + chalk.white(' Logs:    ') + chalk.cyan(`cd ${installDir} && docker-compose logs -f`));
  console.log(chalk.gray('  └─') + chalk.white(' Status:  ') + chalk.cyan(`cd ${installDir} && docker-compose ps`));
  
  console.log(chalk.gray('\n📖 Documentation: ') + chalk.cyan('https://desterlib.github.io/desterlib'));
  console.log('');
}
